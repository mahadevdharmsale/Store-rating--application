...
router.post("/create", async (req, res) => {
  ...
  try {
    ...
    // Fetch the newly created store using insertId
    const [newStoreRows] = await db.query("SELECT * FROM stores WHERE id = ?", [result.insertId]);

    res.status(201).json({
      message: "Store created successfully",
      store: newStoreRows[0] || null,
    });
  } catch (err) {
...
// ✅ Get all stores (with owner name)
router.get("/stores", async (req, res) => {
  try {
    const [rows] = await db.query(`
      SELECT s.id, s.name, s.address, s.owner_id, u.name AS owner_name
      FROM stores s
